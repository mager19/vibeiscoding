...
El resultado es un sistema RAG (Retrieval-Augmented Generation) personalizado: no solo encuentras las notas relevantes, sino que el LLM las lee y sintetiza una respuesta que conecta los puntos entre tus propios escritos. Como tener una conversación con la versión más informada de ti mismo.
...
- **Next.js 15 (App Router)**: Framework base. Server components para la búsqueda, client components para la UI interactiva.
...
- **SDK de IA (Anthropic, OpenAI, etc.)**: Para la síntesis RAG. Los LLMs modernos con ventanas de contexto grandes funcionan muy bien para sintetizar y citar fragmentos de tus notas.
...
2. **Implementar el chunker**: Función que toma el texto de un archivo y lo divide en chunks de ~500 tokens con 50 tokens de overlap entre chunks. Pedile a tu agente que lo implemente con la librería `tiktoken` para contar tokens con precisión.
...
- **Necesitas**: API key de un LLM para la síntesis RAG (Anthropic, OpenAI, etc.) y API key de un proveedor de embeddings (OpenAI, Cohere, Voyage AI). Para la base de datos vectorial, Supabase tiene un free tier con pgvector integrado — la opción más simple para empezar.
...
- **Concepto clave a entender primero**: Qué es un embedding (un vector numérico que representa el significado de un texto). Pídele a tu herramienta de IA que te lo explique con una analogía antes de empezar a construir.
